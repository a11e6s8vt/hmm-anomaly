use anyhow::Result;
use hmm_anomaly::{
    compute_anomaly_score, CliInputs, CpuUtilizationEntry, CsvReader, Hmm, TrainingData,
};
use ndarray::Array2;
...
fn main() -> Result<()> {
    ...
    let _ = train_data.read_csv(input_data_file);
    let test_input = train_data
        .records
        .iter()
        .map(|x| x.utilization)
        .collect::<Vec<_>>();
    ...
    let (transition_samples, emission_means_samples, emission_variances_samples) =
        hmm.learn_gibbs_sampling(&test_input, NUM_ITER, BURN_IN)?;

    hmm.anomalies(&test_input);
